...
### Description
* Problem B : Array Walk [https://codeforces.com/contest/1389/p...] <br>
* Contest Link : Educational Codeforces Round 92 (Rated for Div. 2) [https://codeforces.com/contest/1389] <br>
* Youtube Video Explanation : https://www.youtube.com/watch?v=z7rVnmppk_k <br>
...
void solve(){
    ...
    for (int i = 1; i <= k; ++i)
    {
        ...
        for (int j = 0; j <= z; ++j)
        {
            if(i-2*j < 0)
                break;
            dp[i][j][0] = max(a[i-2*j] + max(dp[i-1][j][1],dp[i-1][j][0]), dp[i][j][0]);
        }
        // Backward
        for (int j = 0; j < z; ++j)
        {
            if(i-2*(j+1) < 0)
                break;
            dp[i][j+1][1] = max(dp[i-1][j][0] + a[i-2*(j+1)], dp[i][j+1][1]);
        }
    }
    ll ans = -1;
    // for (int i = 0; i < n; ++i)
